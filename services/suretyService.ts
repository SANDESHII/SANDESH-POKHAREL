...
import { TeamStats, RegimeState } from './mathUtils';
import { SimulationResult, MarketAudit } from './monteCarloService';
import { MirrorMatch, ProsecutionCase, MarketReality, MatchContext } from './geminiService';
...
/**
 * Extreme Value Theory (EVT) Tail Analysis
 * Models the probability of "Black Swan" events (Extreme Blowouts/Collapses)
 * Uses Steel Data (npxG/xT) to detect structural fragility.
 */
export const calculateEVTRisk = (home: TeamStats, away: TeamStats): number => {
    const combinedSteel = home.npxG + away.npxG;
    const combinedXT = home.xT + away.xT;
    const structuralFragility = (home.avgXGA + away.avgXGA) / 2;
    
    // EVT focuses on the "Fat Tails" of the distribution.
    // High structural intent (xT) + High fragility = High Tail Risk.
    const tailRisk = (combinedXT * combinedSteel) / (1 + structuralFragility);
    return Math.min(100, Math.max(0, tailRisk * 12));
};
...
export const calculateMedallionSurety = (
    ...
    // We tax the score heavily if the Survival Rating in "Disaster Scenarios" is low.
    let suretyScore = (simulation.probability * 0.4) + (simulation.survivalRating * 0.4) + (mirrorSimilarity * 0.2);
