...
export class MonteCarloSimulator {
    static run(hL: number, aM: number, hV: number, aV: number, threshold: number = 1.5, isUnder: boolean = false, rho: number = -0.11, sR: number = 0.05, iters: number = 5000): SimulationResult {
        ...
        const probs = Array.from({ length: iters }, () => {
            ...
            const matrix = DixonColes.calculateScoreMatrix(hLS, aMS, rS);
            const pOver = DixonColes.calculateOverUnder(matrix, threshold);
            return isUnder ? 1 - pOver : pOver;
        });
